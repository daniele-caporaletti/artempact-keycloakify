...
/* -----------------------------------------------------------------------------
    [MODIFICA] Funzione di validazione per data di nascita
-----------------------------------------------------------------------------*/
function validateDob(dobStr: string): string | null {
    if (!dobStr.trim()) {
        return "Inserisci la data di nascita (gg/mm/aaaa).";
    }
    ...
    // Controlli di base (2 cifre giorno/mese, 4 cifre anno, ecc.)
    if (
        isNaN(day) || 
        isNaN(month) || 
        isNaN(year) || 
    ...
    // Costruiamo la data JavaScript
    // Nota: JS usa i mesi 0-based (0=Gennaio, 1=Febbraio, ecc.)
...
export default function UserProfileFormFields(props: UserProfileFormFieldsProps<KcContext, I18n>) {
    ...
    // -------------------------------------------------------------------------
    // 1) useUserProfileForm fornisce isFormSubmittable (senza errori "nativi")
    // -------------------------------------------------------------------------
    const {
    ...
    // -------------------------------------------------------------------------
    // 2) [MODIFICA] Stato locale per l'errore su "dob"
    //    Se "dobError" è NON undefined, disabilitiamo il submit
    // -------------------------------------------------------------------------
    const [dobError, setDobError] = useState<string | undefined>(undefined);

    // -------------------------------------------------------------------------
    // 3) Se c'è un errore di "dob", blocchiamo il form
    // -------------------------------------------------------------------------
    useEffect(() => {
        onIsFormSubmittableValueChange(isFormSubmittable && dobError === undefined);
    }, [isFormSubmittable, dobError]);
                        ...
                        >
                            ...
                            <div className={kcClsx("kcInputWrapperClass")}>
                                <InputFieldByType
                                    ...
                                    kcClsx={kcClsx}
                                    i18n={i18n}
                                    // [MODIFICA] Passa setDobError
                                    setDobError={setDobError}
                                />

                                {/* Errore Keycloak standard */}
                                <FieldErrors attribute={attribute} displayableErrors={displayableErrors} kcClsx={kcClsx} fieldIndex={undefined} />

                                {/* [MODIFICA] Mostra errore dob custom, se presente */}
                                {attribute.name === "dob" && dobError !== undefined && (
                                    <span
...
/* -----------------------------------------------------------------------------
   4) InputFieldByType
   Passiamo setDobError a <InputTag> e lì facciamo la validazione se "name === dob"
-----------------------------------------------------------------------------*/
type InputFieldByTypeProps = {
    ...
    i18n: I18n;
    kcClsx: KcClsx;
    setDobError?: (error: string | undefined) => void; // [MODIFICA]
};
...
/* -----------------------------------------------------------------------------
   5) InputTag: validazione su onBlur (se attribute.name === "dob")
   invece di usare "update error", usiamo setDobError dello stato locale.
-----------------------------------------------------------------------------*/
function InputTag(props: InputFieldByTypeProps & { fieldIndex: number | undefined }) {
    const { attribute, fieldIndex, kcClsx, dispatchFormAction, valueOrValues, i18n, displayableErrors, setDobError } =
        props;
        ...
        <>
            <input
                ...
                onBlur={event => {
                    ...
                    if (attribute.name === "dob" && setDobError) {
                        const errorMsg = validateDob(event.target.value);
                        setDobError(errorMsg ?? undefined);
                    }
                }}
            />
            {(() => {
                ...
                <div key={option} className={classDiv}>
                    ...
                    <label
                        ...
                        className={`${classLabel}${
                            attribute.readOnly ? ` ${kcClsx("kcInputClassRadioCheckboxLabelDisabled")}` : ""
                        }`}
                    >
